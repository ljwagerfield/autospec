...
ThisBuild / githubWorkflowBuild := WorkflowStep.Sbt(List("scalafmtCheckAll", "test"))

lazy val commonSettings = Seq(
  organization := "io.autospec",
  ...
  outputStrategy := Some(
      StdoutOutput
    ) // Send any forked output to stdout/err (instead of to SBT's logger, which can cause annoyances such as "[info] [DEBUG] Message").
)
...
lazy val commonScalaSettings = commonSettings ++: Seq(
    ...
    libraryDependencies ++= Seq(
        ...
        ) // Allows silencing warning for generated files.
      )
  )
